...
class hangman:
    ...
    lives = ['Number of incorrect guesses made:']                           # Players life count
    guess_remaining=['No of incorrect guesses remaining:']
    end_state = False                                                       # Is the game over
    # List create from random word generator
    word_list = ['dream','aeroplane','island','bird']
...
gui_gameboard = tk.Label(main_form, text=game.gameboard ,bg='pink',font = "Verdana 30 bold")
...
def create_button(letter,xpos,ypos,index):
    letter = tk.Button(main_form, text=(alphaList[index].upper()),activebackground='yellow',bg='cyan',command = lambda: btn_Click(letter,alphaList[index].upper()))
    letter.pack()
